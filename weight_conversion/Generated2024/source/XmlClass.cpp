                                          ...
                                          XERCES_CPP_NAMESPACE::XercesDOMParser& aParser)
{
    ...
    {
        std::cerr << "ERROR: Exception occurred: " << Str::ToString(e.getMessage()) << "; File being parsed is " << aFilename << std::endl;
        return Result::ParserError;
    }
 	catch (...)
 	{
        std::cerr << "ERROR: Unexpected Exception occurred: " << aFilename << std::endl;
        return Result::ParserError;
	}
